...
def walk(code):
    ...
    try:
        ...
        pass  # a program may call exit(); compare output produced up to then
    return out.getvalue()
...
class TestVMMatchesInterpreter(unittest.TestCase):
    ...
    def test_examples(self):
        ...
        for path in files:
            ...
            # concurrency.sy uses spawn, which runs on the default engine only.
            if os.path.basename(path) in ("native.sy", "concurrency.sy"):
                continue
