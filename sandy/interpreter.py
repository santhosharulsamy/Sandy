...
class Interpreter:
    def __init__(self, out=None):
        ...
        # Original builtin bindings, to filter module exports. A name counts as
        # user-defined (and is exported) when its current binding differs from
        # the original builtin — so a module's own `fn join` shadows and
        # exports over the builtin, rather than being hidden by it.
        self._builtins = dict(self.globals.vars)
        self.out = out  # optional writer override for print (tests)
        self.base_dir = None       # directory for resolving relative imports
        self.program_args = []     # command-line args exposed via args()
        self.module_cache = {}     # abspath -> Module (shared across imports)
        self.module_loader = None  # engine-specific runner for imported files

    def user_names(self):
        """Top-level names defined by user code (excludes untouched builtins,
        but includes names that shadow a builtin)."""
        return [k for k, v in self.globals.vars.items()
                if k not in self._builtins or self._builtins[k] is not v]
    ...
    def _resolve_import(self, path, line):
        ...
        if os.path.exists(candidate):
            return candidate
        # An installed package dependency (resolved via the project manifest/lock).
        try:
            from . import packages
            pkg = packages.resolve_import(base, path)
            if pkg is not None:
                return pkg
        except Exception:
            pass  # no project / unavailable here: fall through to the stdlib
